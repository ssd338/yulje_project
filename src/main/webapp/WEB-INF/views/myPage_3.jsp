...
$(function(){
   ...
   function checkPassword(password,id){      //비밀번호 문자열 체크 
      ...
      if(!/^(?=.*[a-zA-Z])(?=.*[!@#$%^*+=-])(?=.*[0-9]).{10,15}$/.test(password)){
         swal('','숫자와 영문자, 특수문자 조합으로 10~15자리를 사용해야 합니다.','warning');
         return false;
      }
      // 올바른 조합인지
      ...
      if(checkNumber <0 || checkEnglish <0 || specail <0){
    	  swal('',"숫자와 영문자, 특수문자를 혼용하여야 합니다.",'warning');
         return false;
      }
      //연속된 문자
      if(/(\w)\1\1\1/.test(password)){
    	  swal('','같은 문자를 연속으로 4번 이상 사용하실 수 없습니다.','warning');
         return false;
      }
      if(password.search(id) > -1){
    	  swal('',"비밀번호에 아이디가 포함되었습니다.",'warning');
         return false;
         }
      return true;
   }
   ...
   $("#mypage_title_btn1").click(function(){            //#newpwd1는 새비밀번호 /#newpwd2는 새비밀번호 확인
	if($("#oldpwd").val() !== $("#pwd").val()){			//myPage_3.get에서 가져온 pwd와 입력된 비밀번호를 비교
		swal('',"입력하신 비밀번호는 기존의 비밀번호와 다릅니다.",'warning') 
		 return false;
		}
	   if($("#newpwd1").val() !== $("#newpwd2").val()){   //입력된 새로운 비밀번호와 비밀번호 확인의 값이 같은지 비교해서 다를경우
		   swal('',"입력하신 새비밀번호와 비밀번호 확인이 일치하지 않습니다.",'warning')   
         return false;
      }
